...
Proof.
  ...
  replace (n + 0) with n by trivial. iFrame "z".
Qed.
...
Proof.
  iIntros "x y". iDestruct (guards_transitive_additive _ A B C with "y x") as "z".
  replace (0 + n) with n by trivial. iFrame "z".
Qed.
...
Proof.
  iIntros "x y". iDestruct (guards_transitive_additive _ A B C with "y x") as "z".
  replace (n + m) with (m + n) by lia. iFrame "z".
Qed.
...
Proof.
  iIntros "x y". iDestruct (guards_transitive_additive _ A B C with "x y") as "z".
  replace (0 + n) with (n) by lia. iFrame "z".
Qed.
...
Proof.
  iIntros "x y". iDestruct (guards_transitive_additive _ A B C with "x y") as "z".
  replace (m + (n-m)) with (n) by lia. iFrame "z".
Qed.
...
Proof.
  iIntros "x y". iDestruct (guards_transitive_additive _ A B C with "y x") as "z".
  replace (n - m + m) with (n) by lia. iFrame "z".
Qed.
...
Local Lemma lguards_open_helper (P Q : iProp Σ) (E F : coPset) n
    ...
    : (P &&{F; n}&&> Q) -∗ P ={E, E ∖ F}=∗
      ▷^n |={E ∖ F}=> (Q ∗ (Q ={E ∖ F, E}=∗ P)).
Proof.
  iIntros "#x y". iApply guards_open_later; trivial.
Qed.
...
Local Lemma guards_open_helper (P Q : iProp Σ) (E F : coPset)
    ...
    : (P &&{F}&&> Q) -∗ P ={E, E ∖ F}=∗
      (Q ∗ (Q ={E ∖ F, E}=∗ P)).
Proof.
  iIntros "#x y". iApply guards_open; trivial.
Qed.
