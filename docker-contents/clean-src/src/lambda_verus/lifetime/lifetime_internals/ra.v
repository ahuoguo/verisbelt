...
Inductive lt_atom :=
  | LtAtomSwell : nat → lt_atom
  | LtAtomNotSwell : nat → lt_atom.
  
Global Instance eq_dec_lt_atom : EqDecision lt_atom.
Proof. solve_decision. Qed.

Global Instance countable_lt_atom : Countable lt_atom.
Proof.
  set (enc bs := match bs with
    | LtAtomSwell a => inl (a)
    | LtAtomNotSwell a => inr (a)
    end).
  set (dec y := Some match y with
    | inl (a) => LtAtomSwell a
    | inr (a) => LtAtomNotSwell a
    end).
  refine (inj_countable enc dec _). by intros []. 
Qed.

Inductive BorState :=
  | Borrow : gset lt_atom → gset lt_atom → BorState
  | Unborrow : gset lt_atom → gset lt_atom → gset lt_atom → BorState.
...
Local Definition slice_name := gname.
Definition gmap_bor_state := gmap slice_name BorState.
Definition gmultiset_bor_state := gmultiset (slice_name * BorState).
Definition gmap_props Σ := gmap slice_name (iProp Σ).

Inductive LtRa :=
  | LtOk : (option (gset lt_atom * gset lt_atom)) → gmultiset lt_atom → gset lt_atom →
            option gmap_bor_state → gmultiset_bor_state →
            option (gset lt_atom) → gset lt_atom →
            LtRa
  | LtFail.
  
Instance lt_op : Op LtRa := λ a b , match a, b with
  | LtOk (Some _) _ _ _ _ _ _, LtOk (Some _) _ _ _ _ _ _ => LtFail
  | LtOk _ _ _ (Some _) _ _ _, LtOk _ _ _ (Some _) _ _ _ => LtFail
  | LtOk _ _ _ _ _ (Some _) _, LtOk _ _ _ _ _ (Some _) _ => LtFail
  | LtOk o1 a1 d1 m1 ms1 w1 wp1, LtOk o2 a2 d2 m2 ms2 w2 wp2 =>
      LtOk
        ...
        (ms1 ⊎ ms2)
        (match w1, w2 with Some w, _ => Some w | _, w => w end)
        (wp1 ∪ wp2)
  | _, _ => LtFail
end.

Definition is_not_permanently_dead (sd : gset lt_atom) (bs: BorState) :=
    match bs with
        | Borrow al de => al ## sd
        | Unborrow _ _ _ => True
    end. 

Definition consistent_bs (sd: gset lt_atom) (bs bs': BorState) :=
    match bs, bs' with
...
Definition consistent_modulo (sd : gset lt_atom) (g : gmap_bor_state) (gs: gmultiset_bor_state)
  := (∀ sn bs' bs ,
        g !! sn = Some bs' →
...
Lemma consistent_bs_mono sd sd' bs bs' :
  sd ⊆ sd' → (consistent_bs sd bs bs' → consistent_bs sd' bs bs').
Proof.
...
Lemma is_not_permanently_dead_mono sd sd' bs' :
  sd ⊆ sd' → (is_not_permanently_dead sd' bs' → is_not_permanently_dead sd bs').
Proof.
  unfold is_not_permanently_dead. destruct bs'; set_solver.
Qed.

Lemma consistent_modulo_mono sd sd' g gs :
  sd ⊆ sd' →
  consistent_modulo sd g gs →
  consistent_modulo sd' g gs.
Proof.
  intros Hsub [Hfwd [Hrev Hunique]]. split.
  { intros sn bs' bs. have Hf := Hfwd sn bs' bs.
    have Hx := consistent_bs_mono sd sd' bs bs'.
    have Hy := is_not_permanently_dead_mono sd sd' bs.
    intuition.
  }
  split.
  { intros sn bs Hin Hnotperm.
    assert (is_not_permanently_dead sd bs) as X. {
      apply (is_not_permanently_dead_mono sd sd'); trivial.
    }
  ...
  }
  {
    ...
    have Hy := is_not_permanently_dead_mono sd sd' bs1.
    have Hz := is_not_permanently_dead_mono sd sd' bs2.
    intuition.
  }
Qed.
...
Proof.
  apply consistent_modulo_mono. set_solver.
Qed.
...
Definition lt_inv (a: LtRa) := match a with
  | LtOk (Some (sa, sd)) a d (Some g) gs (Some w) wp =>
      sd = d
      ∧ sa ∩ sd = ∅
      ∧ w = wp
      ∧ w ⊆ d
      ∧ (∀ m , (m ∈ sa → multiplicity m a = 2) ∧ (m ∉ sa → multiplicity m a = 0))
      ∧ consistent_modulo w g gs
  | _ => False
end.
...
Instance lt_pcore : PCore LtRa := λ a , match a with
  | LtOk o1 a1 d1 m1 ms2 w1 wp1 => Some (LtOk None ∅ d1 None ∅ None wp1)
  | LtFail => Some LtFail
end.
...
Proof.
  ...
  destruct x as [[o|] a d [m|] ms [w|] wp|], y as [[o1|] a1 d1 [m1|] ms1 [w1|] wp1|], z as [[o2|] a2 d2 [m2|] ms2 [w2|] wp2|]; trivial; f_equal; (try set_solver); rewrite gmultiset_disj_union_assoc; trivial.
Qed.
  
Lemma lt_comm : Comm (=) lt_op.
Proof.
  unfold Comm. intros x y. unfold lt_op.
  destruct x as [[o|] a d [m|] ms [w|] wp|], y as [[o1|] a1 d1 [m1|] ms1 [w1|] wp1|]; trivial; f_equal; (try set_solver); rewrite gmultiset_disj_union_comm; trivial.
Qed.

Lemma LtOk_incl_3 a b d1 d2 m ms w wp1 wp2
  (d_incl: d1 ⊆ d2)
  (wp_incl: wp1 ⊆ wp2)
  : LtOk a b d1 m ms w wp1 ≼ LtOk a b d2 m ms w wp2.
Proof.
  exists (LtOk None ∅ d2 None ∅ None wp2). destruct a as [a|]; destruct m as [m|]; destruct w as [w|]; unfold "⋅", lt_op; f_equal; try set_solver.
Qed.
  
Lemma dead_subseteq_of_LtOk_incl o0 a0 d0 m0 ms0 w0 wp0 o1 a1 d1 m1 ms1 w1 wp1 (r: LtRa) 
  (incl: LtOk o0 a0 d0 m0 ms0 w0 wp0 ≡ LtOk o1 a1 d1 m1 ms1 w1 wp1 ⋅ r)
  : d1 ⊆ d0.
Proof.
  destruct r as [o2 a2 d2 m2 ms2 w2 wp2|].
  - unfold "⋅", lt_op in incl; destruct o1, o2, m1, m2, w1, w2; inversion incl; set_solver.
  - unfold "⋅", lt_op in incl. destruct o1, m1, w1; inversion incl.
Qed.

Lemma wp_subseteq_of_LtOk_incl o0 a0 d0 m0 ms0 w0 wp0 o1 a1 d1 m1 ms1 w1 wp1 (r: LtRa)
  (incl: LtOk o0 a0 d0 m0 ms0 w0 wp0 ≡ LtOk o1 a1 d1 m1 ms1 w1 wp1 ⋅ r)
  : wp1 ⊆ wp0.
Proof.
  destruct r as [o2 a2 d2 m2 ms2 w2 wp2|].
...
Lemma lemma_Some_g1_eq_g2_of_LtOk_incl (r: LtRa) o1 a1 d1 g1 gs1 w1 wp1 o2 a2 d2 g2 gs2 w2 wp2
  : LtOk o1 a1 d1 (Some g1) gs1 w1 wp1 ⋅ r ≡ LtOk o2 a2 d2 g2 gs2 w2 wp2
      → Some g1 = g2.
Proof.
  intros incl. destruct r as [o3 a3 d3 g3 gs3 w3 wp3|].
  - unfold "⋅", lt_op in incl; destruct o1, o3, g1, g3, w1, w3; inversion incl; trivial.
  - unfold "⋅", lt_op in incl. destruct o1, g1, w1; inversion incl.
Qed.

Lemma lemma_Some_g1_eq_g2_of_LtOk_incl2 (r: LtRa) o1 a1 d1 g1 gs1 w1 wp1 o2 a2 d2 g2 gs2 w2 wp2
  : r ⋅ LtOk o1 a1 d1 (Some g1) gs1 w1 wp1 ≼ LtOk o2 a2 d2 g2 gs2 w2 wp2
      → Some g1 = g2.
Proof.
  intros [t Ha].
  unfold "⋅" in Ha. rewrite (lt_comm r) in Ha. rewrite <- lt_assoc in Ha.
  eapply lemma_Some_g1_eq_g2_of_LtOk_incl. symmetry. apply Ha.
Qed.

Lemma lemma_Some_w1_eq_w2_of_LtOk_incl (r: LtRa) o1 a1 d1 g1 gs1 w1 wp1 o2 a2 d2 g2 gs2 w2 wp2
  : LtOk o1 a1 d1 g1 gs1 (Some w1) wp1 ⋅ r ≡ LtOk o2 a2 d2 g2 gs2 w2 wp2
      → Some w1 = w2.
Proof.
  intros incl. destruct r as [o3 a3 d3 g3 gs3 w3 wp3|].
  - unfold "⋅", lt_op in incl; destruct o1, o3, g1, g3, w1, w3; inversion incl; trivial.
...
Lemma lemma_Some_w1_eq_w2_of_LtOk_incl2 (r: LtRa) o1 a1 d1 g1 gs1 w1 wp1 o2 a2 d2 g2 gs2 w2 wp2
  : LtOk o1 a1 d1 g1 gs1 (Some w1) wp1 ⋅ r ≼ LtOk o2 a2 d2 g2 gs2 w2 wp2
      → Some w1 = w2.
Proof.
  intros [t Ha].
  unfold "⋅" in Ha. rewrite <- lt_assoc in Ha.
  eapply lemma_Some_w1_eq_w2_of_LtOk_incl. symmetry. apply Ha.
Qed.
...
Proof. split.
  ...
  - intros x cx peq. unfold pcore, lt_pcore in peq. destruct x as [o|].
    + inversion peq. subst cx. unfold "⋅", lt_op. destruct o; f_equal; set_solver.
  ...
  - intros x y cx xley peq.
      destruct x as [o a d m ms w wp|], y as [o1 a1 d1 m1 ms1 w1 wp1|]; destruct xley as [[o2 a2 d2 m2 ms2 w2 wp2|] equ];
      unfold pcore, lt_pcore; unfold pcore, lt_pcore in peq; inversion peq; subst cx.
      + exists (LtOk None ∅ d1 None ∅ None wp1); intuition.  apply LtOk_incl_3.
          * eapply dead_subseteq_of_LtOk_incl. apply equ.
          * eapply wp_subseteq_of_LtOk_incl. apply equ.
      + unfold "⋅", lt_op in equ. inversion equ. destruct o, m, w; discriminate.
...
Global Instance lt_unit : Unit LtRa := LtOk None ∅ ∅ None ∅ None ∅.

Definition lt_ucmra_mixin : UcmraMixin LtRa.
split; trivial.
  - exists (LtOk (Some (∅, ∅)) ∅ ∅ (Some ∅) ∅ (Some ∅) ∅). unfold lt_inv. simpl.
      unfold consistent_modulo. set_solver.
  - intro x. destruct x as [o a d|]; simpl; trivial.
      destruct o; trivial.
      + unfold ε, lt_unit, "⋅", lt_op. f_equal; set_solver.
      + unfold ε, lt_unit, "⋅", lt_op. f_equal; set_solver.
Qed.

Canonical Structure ltUR := Ucmra LtRa lt_ucmra_mixin.

Definition outlives_set := gset (gset lt_atom * lt_atom).
  
Class llft_logicGS Σ := {
  llft_logic_inG : inG Σ ltUR ;
  exclG : inG Σ (exclR unitO) ;
  outlivesG : inG Σ (dfrac_agreeR (leibnizO outlives_set)) ;
  delayedG : inG Σ (dfrac_agreeR (leibnizO (option (lt_atom * gset lt_atom)))) ;
}.
...
Definition llft_logicΣ : gFunctors := #[
  ...
  GFunctor (dfrac_agreeR (leibnizO (option (lt_atom * gset lt_atom))))
].

Global Instance subG_lt_logicΣ Σ : subG llft_logicΣ Σ → llft_logicGS Σ.
Proof.
  solve_inG.
Qed.
...
Section LlftHelperResources.
  ...
  Definition lt_state (sa sd : gset lt_atom) := LtOk (Some (sa, sd)) ∅ sd None ∅ None ∅.
  Definition LtState (γlt: gname) (sa sd : gset lt_atom) : iProp Σ := own γlt (lt_state sa sd).

  Definition dead (k: lt_atom) := LtOk None ∅ ({[ k ]}) None ∅ None ∅.
  Definition Dead γlt (k: lt_atom) : iProp Σ := own γlt (dead k).

  Definition alive (k: lt_atom) := LtOk None ({[+ k +]}) ∅ None ∅ None ∅.
  Definition Alive γlt (k: lt_atom) : iProp Σ := own γlt (alive k).
  
  Definition auth_bor_state (mbs: gmap_bor_state) := LtOk None ∅ ∅ (Some mbs) ∅ None ∅.
  Definition own_bor_state (sn: slice_name) (bs: BorState) := LtOk None ∅ ∅ None {[+ (sn, bs) +]} None ∅.
  
  Definition modulo_dead (w: gset lt_atom): LtRa := LtOk None ∅ w None ∅ (Some w) w.
  Definition modulo_dead_pers (wp: lt_atom): LtRa := LtOk None ∅ {[ wp ]} None ∅ None ({[ wp ]}).
  ...
  Definition ModuloDead (γlt: gname) (g: gset lt_atom) : iProp Σ :=
      own γlt (modulo_dead g).
      
  Definition ModuloDeadPers (γlt: gname) (g: lt_atom) : iProp Σ :=
      own γlt (modulo_dead_pers g).
  
  Definition default_dead : lt_atom := LtAtomSwell 0.
  
  Lemma lt_alloc :
    ⊢ |==> ∃ γ , LtState γ ∅ ∅ ∗ AuthBorState γ ∅ ∗ ModuloDead γ ∅.
  Proof.
    unfold LtState, AuthBorState.
    setoid_rewrite <- own_op.
    setoid_rewrite <- own_op.
    apply own_alloc. unfold "✓", cmra_valid, ltR, lt_valid.
    exists (LtOk None ∅ ∅ None ∅ None ∅). simpl. unfold consistent_modulo. set_solver.
  Qed.
  ...
  Lemma update_helper2 (x y : LtRa)
    (cond: ∀ o a d m ms w wp , lt_inv (x ⋅ LtOk o a d m ms w wp) → lt_inv (y ⋅ LtOk o a d m ms w wp))
    : x ~~> y.
  Proof.
    ...
    - intros lti. unfold lt_inv in lti.
      ...
      destruct x as [o a d m ms w wp|]; trivial. destruct o, m, w; trivial.
  Qed.
  ...
  Proof.
    ...
    intros o a d g gs w wp lti. destruct o as [[sa sd]|], g as [g|], w as [w|]; try contradiction.
      simpl in lti. destruct lti as [sd_d [no_int [wp_p [wsub [fm cm]]]]].
  ...
  Lemma alloc_bor_state2 γlt sn mbs sd al de de' :
      (mbs !! sn = None) →
      (de = de' ∨ (¬(de' ## sd) ∧ ¬(de ## sd))) →
      ModuloDead γlt sd ∗ AuthBorState γlt mbs
      ⊢ |==> ModuloDead γlt sd ∗ AuthBorState γlt (<[ sn := Borrow al de' ]> mbs) ∗ OwnBorState γlt sn (Borrow al de).
  Proof.
    intros Hnone Hconsistent.
    setoid_rewrite <- own_op. setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
    intros o a d g gs w wp lti. destruct o as [[sa' sd']|], g as [g|], w as [w|]; try contradiction.
      simpl in lti. destruct lti as [sd_d [wp_p [wsub [no_int [fm cm]]]]].
      simpl. split. { set_solver. } split. { set_solver. } split. { set_solver. }
  ...
  Lemma alloc_fake_bor_state_lts (γlt: gname) (sn: slice_name) (sa sd: gset lt_atom) (alive dead : gset lt_atom) :
      ¬(alive ## sd) →
      ModuloDead γlt sd
       ⊢ |==> ModuloDead γlt sd ∗ OwnBorState γlt sn (Borrow alive dead).
  Proof.
    intros Hnotdisj.
    setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
  ...
  Lemma update_bor_state_borrow_fix_dead_helper γlt sn al de de' k k' :
      (k ∈ de) →
      (k' ∈ de') →
      ModuloDeadPers γlt k ∗ ModuloDeadPers γlt k' ∗ OwnBorState γlt sn (Borrow al de) ==∗ ModuloDeadPers γlt k ∗ ModuloDeadPers γlt k' ∗ OwnBorState γlt sn (Borrow al de').
  Proof.
    intros Hk Hk'.
    setoid_rewrite <- own_op. setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
  ...
  Lemma update_bor_state_borrow_fix_dead γlt sn al de de' k k' :
      (k ∈ de) →
      (k' ∈ de') →
      ModuloDeadPers γlt k ∗ ModuloDeadPers γlt k' ∗ OwnBorState γlt sn (Borrow al de) ==∗ OwnBorState γlt sn (Borrow al de').
  Proof.
  ...
  Lemma update_bor_state_borrow_fix_dead_lts γlt sd sn al de de' :
      ¬(de ## sd) →
      ¬(de' ## sd) →
      ModuloDead γlt sd ∗ OwnBorState γlt sn (Borrow al de) ==∗ ModuloDead γlt sd ∗ OwnBorState γlt sn (Borrow al de').
  Proof.
    intros Hk Hk'.
    setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
  ...
  Lemma update_bor_state_borrow_lts γlt sn mbs sd alive dead bs' :
      (alive ## sd) →
      ModuloDead γlt sd ∗ AuthBorState γlt mbs ∗ OwnBorState γlt sn (Borrow alive dead)
      ⊢ |==> ModuloDead γlt sd ∗ AuthBorState γlt (<[ sn := bs' ]> mbs) ∗ OwnBorState γlt sn bs'.
  Proof.
    intros Hdisj.
    setoid_rewrite <- own_op. setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
  ...
  Lemma update_bor_state_unborrow_lts γlt sn mbs sd bl alive dead bs' :
      ModuloDead γlt sd ∗ AuthBorState γlt mbs ∗ OwnBorState γlt sn (Unborrow bl alive dead)
      ⊢ |==> ModuloDead γlt sd ∗ AuthBorState γlt (<[ sn := bs' ]> mbs) ∗ OwnBorState γlt sn bs'.
  Proof.
    setoid_rewrite <- own_op. setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
    intros o a d g gs w wp lti. destruct o as [[sa' sd']|], g as [g|], w as [w|]; try contradiction.
      ...
      rewrite gmultiset_disj_union_left_id. rewrite gmultiset_disj_union_left_id.
      rewrite gmultiset_disj_union_left_id in cm. rewrite gmultiset_disj_union_left_id in cm.
      apply consistent_modulo_for_update with (bs1 := Unborrow bl alive dead); trivial.
      unfold is_not_permanently_dead. trivial.
  Qed.
  
  Lemma delete_bor_state_borrow_lts γlt sn mbs sd al de :
      (al ## sd) →
      ModuloDead γlt sd ∗ AuthBorState γlt mbs ∗ OwnBorState γlt sn (Borrow al de)
        ⊢ |==> 
      ModuloDead γlt sd ∗ AuthBorState γlt (delete sn mbs).
  Proof.
    intros Hdisj.
    setoid_rewrite <- own_op. setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
  ...
  Lemma own_bor_state_borrow_modulo_dead_and γlt sn al de sd mbs :
      OwnBorState γlt sn (Borrow al de) ∧ (ModuloDead γlt sd ∗ AuthBorState γlt mbs)
      ⊢ OwnBorState γlt sn (Borrow al de) ∗ (ModuloDead γlt sd ∗ AuthBorState γlt mbs).
  Proof.
    unfold OwnBorState, ModuloDead, AuthBorState, own_bor_state, modulo_dead, auth_bor_state.
   rewrite <- own_op. rewrite <- own_op.
   iApply and_own_discrete_ucmra_specific.
   intros z Hvalz Hincl1 Hincl2.
   destruct z as [o a d g gs w wp|].
   - unfold "⋅", cmra_op, ltR, lt_op.
   
     destruct Hincl1 as [[o1 a1 d1 g1 gs1 w1 wp1|] Hincl1]; last by inversion Hincl1.
     
     assert (sd ⊆ d) as Hsubdead. {
      destruct Hincl2 as [t Hincl2]. rewrite <- lt_assoc in Hincl2.
      eapply dead_subseteq_of_LtOk_incl. apply Hincl2.
     }
     assert (sd ⊆ wp) as Hsubwp. {
      destruct Hincl2 as [t Hincl2]. rewrite <- lt_assoc in Hincl2.
      eapply wp_subseteq_of_LtOk_incl. apply Hincl2.
     }
   
     exists (LtOk o a d None gs1 None wp).
     unfold "⋅", cmra_op, ltUR, ucmra_cmraR, ucmra_op, lt_op.
     f_equal; trivial.
     + set_solver.
     + set_solver.
     + symmetry. eapply lemma_Some_g1_eq_g2_of_LtOk_incl2. apply Hincl2.
     + inversion Hincl1. set_solver.
     + symmetry. eapply lemma_Some_w1_eq_w2_of_LtOk_incl2. apply Hincl2.
     + set_solver.
   - inversion Hvalz. destruct x; contradiction.
  Qed.
  
  Lemma agree_bor_state_borrow_lts γlt sn mbs (sd al de : gset lt_atom) :
      (al ## sd) →
      ModuloDead γlt sd
  ...
  Proof.
    ...
    destruct b as [o a d g gs w wp|]; try contradiction.
    destruct o as [[sa' sd']|], g as [g|], w as [w|]; try contradiction.
    simpl in lti. destruct lti as [sd_d [no_int [wp_p [wsub [fm cm]]]]].
    rewrite gmultiset_disj_union_left_id in cm. rewrite gmultiset_disj_union_left_id in cm.
  ...
  Lemma agree_bor_state_borrow_lts_and γlt sn mbs (sd al de : gset lt_atom) :
      (al ## sd) →
      OwnBorState γlt sn (Borrow al de) ∧ (ModuloDead γlt sd ∗ AuthBorState γlt mbs)
      ⊢ ∃ de' , ⌜mbs !! sn = Some (Borrow al de') ∧ (de = de' ∨ (¬(de' ## sd) ∧ ¬(de ## sd)))⌝.
  Proof.
      iIntros (Hdisj) "And".
      iDestruct (own_bor_state_borrow_modulo_dead_and with "And") as "[A [B C]]".
      iApply agree_bor_state_borrow_lts. { trivial. } iFrame.
  Qed.
  ...
  Proof.
    ...
    destruct b as [o a d g gs w wp|]; try contradiction.
    destruct o as [[sa sd]|], g as [g|], w as [w|]; try contradiction.
    simpl in lti. destruct lti as [sd_d [no_int [wp_w [wsub [fm cm]]]]].
    rewrite gmultiset_disj_union_left_id in cm.
    destruct (consistent_modulo_for_agree sd mbs gs sn (Unborrow bl al de)) as [bs' x]; trivial.
    { unfold is_not_permanently_dead. trivial. }
    { apply (consistent_modulo_mono w sd); trivial. set_solver. }
    destruct bs' as [|bl' al' de'].
    - unfold consistent_bs in x. destruct x as [Ha Hb]. discriminate.
    - unfold consistent_bs in x. destruct x as [Ha Hb]. rewrite Hb. trivial.
  Qed.
      
  Lemma distinct_bor_state_borrow_borrow_lts γlt sn1 sn2 al1 de1 al2 de2 sd :
      (al1 ## sd) →
      (al2 ## sd) →
      ModuloDead γlt sd
  ...
  Local Instance pers_modulo_dead2 γlt k : Persistent (ModuloDeadPers γlt k).
  Proof.
    apply own_core_persistent. unfold CoreId. trivial.
  Qed.
  ...
  Proof.
    ...
    - iIntros (d) "%Hsd".
      replace (LtOk (Some (sa, sd)) ∅ sd None ∅) with (LtOk (Some (sa, sd)) ∅ sd None ∅ None ∅ ⋅ LtOk None ∅ {[d]} None ∅ None ∅).
  ...
  *)
  ...
  Lemma get_all_modulo_dead_pers γlt sd :
      ModuloDead γlt sd ⊢ ModuloDead γlt sd ∗ (∀ d , ⌜d ∈ sd⌝ -∗ ModuloDeadPers γlt d).
  Proof.
    unfold LtState, lt_state. iIntros "o". 
    iSplit.
    - iFrame.
    - iIntros (d) "%Hin". unfold ModuloDead, modulo_dead, ModuloDeadPers, modulo_dead_pers.
      replace (LtOk None ∅ sd None ∅ (Some sd) sd) with
        (LtOk None ∅ sd None ∅ (Some sd) sd ⋅ LtOk None ∅ {[d]} None ∅ None {[d]}).
      + iDestruct "o" as "[o p]". iFrame "p".
      + unfold "⋅". simpl. f_equal; set_solver.
  Qed.
  
  Lemma modulo_dead_pers_implies_dead γlt d :
      ModuloDeadPers γlt d -∗ Dead γlt d.
  Proof.
      unfold ModuloDeadPers, modulo_dead_pers, Dead, dead.
      iApply own_mono.
      exists (LtOk None ∅ {[d]} None ∅ None {[d]}).
      unfold "⋅", cmra_op, ltR. simpl. f_equal; set_solver.
  Qed.
      
  Lemma modulo_dead_new γlt k sd' :
      Dead γlt k ∗ ModuloDead γlt sd' ==∗
      ModuloDead γlt (sd' ∪ {[k]}).
  Proof.
    setoid_rewrite <- own_op.
    iIntros "a". iApply own_update. 2: { iFrame "a". } apply update_helper2.
    intros o a d g gs w wp lti. destruct o as [[sa sd]|], g as [g|], w as [w|]; try contradiction.
      ...
      rewrite gmultiset_disj_union_left_id.
      rewrite gmultiset_disj_union_left_id in cm.
      apply (consistent_modulo_for_kill_lt sd'). trivial.
  Qed.
  ...
  Proof.
    ...
    intros o a d g gs w wp lti. destruct o as [s|], g as [g|], w as [w|]; try contradiction.
    - simpl in lti. destruct lti as [sd_d [no_int [wp_p [wsub [fm cm]]]]].
  ...
  Lemma lt_state_incl_lt_ok sa sd sa1 sd1 a d g gs w wp
    : (lt_state sa sd ≼ LtOk (Some (sa1, sd1)) a d g gs w wp) → sa = sa1 ∧ sd = sd1.
  Proof.
  ...
  Lemma alive_incl_lt_ok k sa1 sd1 a d g gs w wp
    : (alive k ≼ LtOk (Some (sa1, sd1)) a d g gs w wp) → lt_inv (LtOk (Some (sa1, sd1)) a d g gs w wp) → k ∈ sa1.
  Proof.
    ...
    - unfold alive in lts. unfold "⋅", lt_op in lts. inversion lts.
        ...
        destruct lti as [_ [_ [_ [_ [fm cm]]]]]. have fmk := fm k.
  ...
  Lemma dead_incl_lt_ok k sa1 sd1 a d g gs w wp
    : (dead k ≼ LtOk (Some (sa1, sd1)) a d g gs w wp) → lt_inv (LtOk (Some (sa1, sd1)) a d g gs w wp) → k ∈ sd1.
  Proof.
    intros lts lti. destruct lts as [t lts].
    destruct t as [[o|] a1 d1|].
    - unfold dead in lts. unfold "⋅", lt_op in lts. inversion lts.
        unfold lt_inv in lti. destruct g; try contradiction. destruct w; try contradiction.
        destruct lti as [sde _]. subst d. set_solver.
    - inversion lts.
    - inversion lts.
  Qed.

  Lemma alive_dead_contradiction k sa sd a d g gs w wp
    : (lt_inv (LtOk (Some (sa, sd)) a d g gs w wp)) → (k ∈ sa) → (k ∈ sd) → False.
  Proof.
    intros lts lti.
    destruct g; try contradiction. destruct w; try contradiction.
  ...
  Proof.
    ...
    have j := lt_state_incl_lt_ok _ _ _ _ _ _ _ _ _ _ inc1. destruct j as [j1 j2]. subst sa.
    apply (alive_incl_lt_ok _ _ _ _ _ _ _ _ _ inc2). apply lti.
  Qed.
  ...
  Proof.
    ...
    destruct di as [z [inc1 [inc2 lti]]].
    destruct z as [[[sa1 sd1]|] a d|]; try contradiction.
    have j := lt_state_incl_lt_ok _ _ _ _ _ _ _ _ _ _ inc1. destruct j as [j1 j2]. subst sd.
    apply (dead_incl_lt_ok _ _ _ _ _ _ _ _ _ inc2). apply lti.
  Qed.
  ...
  Proof.
    ...
    destruct di as [z [inc1 [inc2 lti]]].
    destruct z as [[[sa1 sd1]|] a d|]; try contradiction.
    have j1 := alive_incl_lt_ok _ _ _ _ _ _ _ _ _ inc1.
    have j2 := dead_incl_lt_ok _ _ _ _ _ _ _ _ _ inc2.
  ...
  Proof.
    ...
    destruct Hv as [b Hv]. destruct b as [[[sa sd]|] a d [g|] gs [w|] wp|].
    - unfold lt_inv in Hv. destruct Hv as [Hsd [Hdisj [Hwp_w [Hwsub [Hfm Hcm]]]]].
  ...
  Lemma bigSepS_alive_equiv_own γlt a
    (ne_emp: a ≠ ∅)
      : ([∗ set] k ∈ a, Alive γlt k) ⊣⊢ own γlt (LtOk None (list_to_set_disj (elements a)) ∅ None ∅ None ∅).
  Proof.
    ...
    - have h : Decision (T = ∅) by solve_decision. destruct h as [h|n]; trivial.
      ...
      + rewrite big_sepS_union; last by set_solver.
        rewrite (IH n). rewrite big_sepS_singleton. unfold Alive, alive.
          rewrite <- own_op.
          f_equiv.
          unfold "⋅", cmra_op, ltR, lt_op. f_equal.
           * rewrite elements_union_singleton; trivial.
  Qed.

  Lemma lt_ok_none_left o a1 d1 a2 d2 g gs1 gs2 wp1 w wp2
      : LtOk None a1 d1 None gs1 None wp1 ⋅ LtOk o a2 d2 g gs2 w wp2 = LtOk o (a1 ⊎ a2) (d1 ∪ d2) g (gs1 ⊎ gs2) w (wp1 ∪ wp2).
  Proof.
    destruct o as [o|], w as [w|]; trivial.
  Qed.

  Lemma mult_list_to_set_disj_not_in (a: gset lt_atom) (x: lt_atom)
  ...
  Lemma mult_list_to_set_disj_in (a: gset lt_atom) (x: lt_atom)
  ...
  Lemma multiplicity_le o1 a1 d1 g1 gs1 w1 wp1 o2 a2 d2 g2 gs2 w2 wp2 x
    : LtOk o1 a1 d1 g1 gs1 w1 wp1 ≼ LtOk o2 a2 d2 g2 gs2 w2 wp2 → multiplicity x a1 ≤ multiplicity x a2.
  Proof.
    intro incl. destruct incl as [t incl]. destruct t as [o3 a3 d3 g3 gs3 w3 sp3|].
    - assert (a2 = a1 ⊎ a3) as X.
        + unfold "⋅", lt_op in incl. destruct o1; destruct o3; destruct w1; destruct w3;
            try (destruct g1, g3; inversion incl; trivial).
        + subst a2. rewrite multiplicity_disj_union. lia.
    - unfold "⋅", lt_op in incl. destruct o1, g1, w1; inversion incl.
  Qed.

  Lemma own_and_alive γlt (a1 a2 : gset lt_atom)
    : own γlt (LtOk None (list_to_set_disj (elements a1)) ∅ None ∅ None ∅)
      ∧ own γlt (LtOk None (list_to_set_disj (elements a2)) ∅ None ∅ None ∅)
      ⊢ own γlt (LtOk None (list_to_set_disj (elements (a1 ∪ a2))) ∅ None ∅ None ∅).
  Proof.
    iIntros "x".
    iDestruct (and_own_discrete_ucmra_specific with "x") as "y"; last by iFrame "y".
    intros w valw incl1 incl2.
    destruct w as [o a d g gs w wp|].
    - exists (LtOk o (a ∖ (list_to_set_disj (elements (a1 ∪ a2)))) d g gs w wp).
      rewrite lt_ok_none_left. f_equal.
      + rewrite gmultiset_eq. intros x.
          ...
          { lia. }
          have t1 := multiplicity_le _ _ _ _ _ _ _ _ _ _ _ _ _ _ x incl1.
          have t2 := multiplicity_le _ _ _ _ _ _ _ _ _ _ _ _ _ _ x incl2.
  ...
  Lemma alive_and_bigSepS γlt (a1 a2 : gset lt_atom)
  ...
  Proof.
    ...
    destruct t as [[o|] a d m1 ms1 w1 wp1|].
    - inversion lts.
    - simpl in lts. intuition. destruct m1, w1; intuition.
    - inversion lts.
  Qed.
  ...
  Lemma not_subset_eq_get (a b : gset lt_atom) : (a ⊈ b) → ∃ k , k ∈ a ∧ k ∉ b.
  Proof.
    assert (∀ r , list_to_set r ⊈ b → ∃ u: lt_atom , u ∈ ((list_to_set r) : gset lt_atom) ∧ u ∉ b) as X.
  ...
  Definition Delayed γd (opt_k: option (lt_atom * gset lt_atom)) : iProp Σ :=
      own γd (to_frac_agree (A:=leibnizO (option (lt_atom * gset lt_atom))) (1/2)%Qp opt_k).
  ...
  Proof.
      unfold Outlives.
      iMod (own_alloc
          (to_frac_agree (A:=leibnizO (option (lt_atom * gset lt_atom))) (1 / 2) d ⋅ to_frac_agree (A:=leibnizO (option (lt_atom * gset lt_atom))) (1 / 2) d)) as (γd) "[Ho Ho2]".
        - rewrite frac_agree_op_valid. rewrite Qp.half_half. split; trivial.
  ...
  Lemma delayed_update γd (d1 d2 d : option (lt_atom * gset lt_atom)) :
