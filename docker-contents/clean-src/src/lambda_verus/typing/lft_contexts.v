...
Definition elctx_elt_interp `{!invGS Σ, !llft_logicGS Σ} (x : elctx_elt) : iProp Σ :=
  (x.1 ⊑ x.2)%I.
...
Section lft_contexts.
  ...
  Global Instance llctx_elt_interp_timeless (x: llctx_elt) : Timeless (llctx_elt_interp x).
  Proof.
    typeclasses eauto.
  Qed.
  ...
  *)
  ...
  Global Instance llctx_interp_timeless (L: llctx) : Timeless (llctx_interp L).
  Proof.
    typeclasses eauto.
  Qed.
  ...
  *)
  ...
  Proof.
    ...
    iMod (lft_eternalize with "Hκ") as "#Hincl". { solve_ndisj. }
    iModIntro. iSplit.
    - iApply llftl_incl_trans; iApply llftl_intersect_incl_l.
    - iApply (llftl_incl_glb with "[]"); first iApply (llftl_incl_glb with "[]").
      + iApply lft_incl_refl.
      + iApply lft_incl_static.
      + iApply llftl_incl_trans; last done. iApply lft_incl_static.
  Qed.
  
  Lemma llctx_interp_make_guarded `{fractional.frac_logicG Σ} (L : llctx) (E: coPset) :
    (↑NllftG ⊆ E) →
    llctx_interp L ⊢ |={E}=> ∃ (γ: gname) ,
        fractional.half γ ∗ fractional.half γ ∗ (fractional.half γ &&{↑NllftG}&&> llctx_interp L)
          ∗ □ (fractional.half γ -∗ fractional.half γ ={↑NllftG}=∗ llctx_interp L).
  Proof.
    iIntros (?) "L". iApply (fractional.frac_split_guard_in_half with "L"); trivial.
    iApply guards_refl.
  Qed.
  ...
  Proof.
    ...
    iClear "∗". iIntros "!> #HE". iApply guards_transitive.
  ...
  Proof.
    ...
    iApply llftl_incl_trans; first by iApply llftl_intersect_incl_l.
  ...
  Proof. iIntros "**". rewrite -llftl_tok_unit_equiv. iApply guards_true. Qed.
  ...
  Lemma lctx_lft_alive_intersect α β :
    ...
    lctx_lft_alive (α ⊓ β).
  Proof. 
    iIntros (alα alβ) "L E". iDestruct (alα with "L E") as "#gα".
    iDestruct (alβ with "L E") as "#gβ". 
    iApply guards_and_point; [
      exact: point_prop_llft_alive|by rewrite llftl_tok_inter_and|done..].
  Qed.
  ...
  Proof.
    elim; [exact: lctx_lft_alive_static|]=>/= ????. 
    by apply lctx_lft_alive_intersect.
  Qed.

  Lemma lctx_lft_alive_local κ κs:
    κ ⊑ₗ κs ∈ L → Forall lctx_lft_alive κs → lctx_lft_alive κ.
  Proof.
    unfold lctx_lft_alive. intros Hin Hf.
    have H2 := lctx_lft_alive_tok_list _ Hf.
    iIntros "HL #HE".
    ...
    iApply (guards_and_point with "Alv HL1"). { apply point_prop_llft_alive. }
    iIntros "AB".
    unfold llctx_elt_interp. rewrite bi.and_exist_l. iDestruct "AB" as (κ') "AB".
    simpl. 
    iAssert (⌜κ = foldr meet static κs ⊓ κ'⌝)%I with "[AB]" as "%Hk". 
      { iDestruct "AB" as "[_ [$ _]]". } subst κ.
    rewrite llftl_tok_inter_and. iSplit.
     - iDestruct "AB" as "[$ _]".
     - iDestruct "AB" as "[_ (_ & $ & _)]".
  Qed.
  ...
  Proof.
    ...
    iApply (guards_transitive with "Ha Hb").
  Qed.
...
Lemma elctx_sat_submseteq `{!invGS Σ, !llft_logicGS Σ} E E' L :
  E' ⊆+ E → elctx_sat E L E'.
Proof. iIntros (HE') "_ !> H". by iApply big_sepL_submseteq. Qed.

Lemma lctx_lft_alive_get_guards `{!invGS Σ, !llft_logicGS Σ} (G: iProp Σ) E L F κ :
  lctx_lft_alive E L κ →
  ↑NllftG ⊆ F →
  ((G &&{ ↑NllftG }&&> llctx_interp L)
    -∗ ([∗ list] x ∈ E, elctx_elt_interp x)
    -∗ G
    -∗ |={F}=> G ∗ (llctx_interp L &&{↑NllftG}&&> @[κ])).
Proof.
    intros Halive Hmask. iIntros "#guards E G".
    leaf_open "guards" with "G" as "[L back]". { trivial. }
    iDestruct (Halive with "L E") as "#X". iMod ("back" with "L") as "G". iModIntro.
    iFrame "X". iFrame "G".
Qed.
...
Global Hint Resolve
     ...
     lctx_lft_alive_static lctx_lft_alive_external lctx_lft_alive_local
