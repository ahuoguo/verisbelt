...
Fixpoint psum_map1 {A} {F G: A → Type} (f: ∀ A , F A → G A) {Xl} :
   psum F Xl → psum G Xl :=
  match Xl with
  | [] => absurd
  | _::_ => sum_map (f _) (psum_map1 f)
  end.

Lemma psum_map1_pinj {A} {F G: A → Type} (f: ∀ A , F A → G A) {Xl: list A} i x :
  psum_map1 f (pinj i x) = @pinj _ _ Xl i (f _ x).
Proof.
  move: i x. elim: Xl=>/=. { move=> i. inv_fin i. }
  move=> ?? IH i. inv_fin i=>//= ??. by rewrite IH.
Qed.
...
Lemma concat_fmap_nil {A B: Type} (x: list A) : concat ((λ (_ : A), ([] : list B)) <$> x) = ([] : list B).
Proof.
  induction x; done.
Qed.
