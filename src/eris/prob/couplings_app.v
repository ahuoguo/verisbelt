...
From eris.prelude Require Export base Coquelicot_ext Reals_ext stdpp_ext.
From eris.prob Require Export countable_sum distribution couplings graded_predicate_lifting.
...
Section couplings_theory.
  ...
  (* Depend on both *)
  Lemma ARcoupl_dbind_adv_kanto_plain (f : A → distr A') (g : B → distr B')
    ...
    (forall h1 h2,
        (forall a : A, 0 <= h1 a <= 1) ->
        (forall b : B, 0 <= h2 b <= 1) ->
        (forall a b, h1 a <= h2 b + E2 a b) ->
        (SeriesC (λ a, μ1 a * h1 a) <=
           SeriesC (λ b, μ2 b * h2 b) + ε2)) ->
    (∀ a b, ARcoupl (f a) (g b) S' (E2 a b)) → ARcoupl (dbind f μ1) (dbind g μ2) S' (ε2).
  Proof.
    intros HE2 Hε2 Hcoup_fg h1 h2 Hh1pos Hh2pos Hh1h2S.
  ...
  Lemma ARcoupl_dbind_adv_kanto_full (f : A → distr A') (g : B → distr B')
    ...
    (forall h1 h2,
        (forall a : A, 0 <= h1 a <= 1) ->
        (forall b : B, 0 <= h2 b <= 1) ->
        (forall a b, S a b -> h1 a <= h2 b + E2 a b) ->
        (SeriesC (λ a, μ1 a * h1 a) <=
           SeriesC (λ b, μ2 b * h2 b) + ε2)) ->
    (ARcoupl μ1 μ2 S ε1) ->
    (∀ a b, S a b -> ARcoupl (f a) (g b) S' (E2 a b)) → ARcoupl (dbind f μ1) (dbind g μ2) S' ε2.
  Proof.
    intros Hε1 HE2 Hε2 HcoupR Hcoup_fg.
    clear HcoupR.
    eapply (ARcoupl_dbind_adv_kanto_plain _ _ _ _ _ _ (λ a b, if bool_decide (S a b) then E2 a b else 1)).
    - real_solver.
    - intros h1 h2 Hh1 Hh2 Hh12.
      transitivity (SeriesC (λ b : B, μ2 b * h2 b) + ε2); [|real_solver].
      apply Hε2; auto.
      intros a b Sab.
      specialize (Hh12 a b).
  ...
  Lemma ARcoupl_adv_kanto_invert (μ1 : distr A) (μ2 : distr B) (S : A -> B -> Prop) ε1 ε2:
    (Rle 0 ε1) -> (Rle 0 ε2) ->
    ARcoupl μ1 μ2 S ε1 ->
    ∀ h1 h2,
      (∀ a, 0 <= h1 a <= 1)
      → (∀ b, 0 <= h2 b <= 1)
      → (∀ a b, S a b -> h1 a <= h2 b + ε2)
      → Expval μ1 h1 <= Expval μ2 h2 + (ε1 + ε2).
  Proof.
    intros Hε1 Hε2 Hcpl h1 h2 Hh1 Hh2 Hh1h2.
    destruct (Rle_lt_dec ε2 1); last first.
    {
      transitivity (0+1).
      - rewrite Rplus_0_l.
        transitivity (SeriesC μ1); auto.
    ...
    etrans.
    + apply (Hcpl h1 (λ ρ, Rmin 1 (h2 ρ + ε2))); auto.
      * intros; split.
        ** apply Rmin_glb.
           *** real_solver.
        ...
        ** apply Rmin_l.
      * intros a b Sab.
        specialize (Hh1h2 a b Sab).
        apply Rmin_glb; real_solver.
    + rewrite /Expval /=.
      replace (SeriesC (λ a, μ2 a * h2 a) + (ε1 + ε2))
               with (SeriesC (λ a, μ2 a * h2 a) + ε2 + ε1) by lra.
      apply Rplus_le_compat_r.
      transitivity (SeriesC (λ a, μ2 a * h2 a) +
                      SeriesC (λ a, μ2 a * ε2)).
      * rewrite -SeriesC_plus.
        ** apply SeriesC_le.
           *** intros ρ; split.
               ...
               **** rewrite Rcomplements.Rmult_min_distr_l; auto.
                    etrans; [apply Rmin_r|].
                    lra.
  ...
  Proof.
    intros Hε1 HE2 Hε2 Hcoup_fg Hcoup_R.
    apply (ARcoupl_dbind_adv_kanto_plain _ _ _ _ S' _ (λ a b, if bool_decide (S a b) then (E2 a) else 1)); auto.
    - real_solver.
    - intros h1 h2 Hh1 Hh2 Hh1h2.
      rewrite -Hε2.
      assert
        (SeriesC (λ a : A, μ1 a * h1 a) - SeriesC (λ a : A, μ1 a * E2 a) <= SeriesC (λ b : B, μ2 b * h2 b) + ε1); last by lra.
      rewrite -SeriesC_minus.
      + rewrite (SeriesC_ext _ (λ x : A, μ1 x * (h1 x - E2 x))); [|real_solver].
        transitivity (SeriesC (λ x : A, μ1 x * (Rmax 0 (h1 x - E2 x)))).
        * apply SeriesC_le'.
          ** intros a.
             apply Rmult_le_compat_l; auto.
             apply Rmax_r.
          ** apply (ex_seriesC_ext (λ x : A, μ1 x * h1 x + (-1 * (μ1 x * E2 x)))); [real_solver|].
             apply ex_seriesC_plus.
             *** apply (ex_seriesC_le _ μ1); auto.
                 real_solver.
 ...
 Qed.
  ...
  Proof.
    intros Hε1 HE2 Hε2 Hcoup_fg Hcoup_R.
    apply (ARcoupl_dbind_adv_kanto_plain _ _ _ _ S' _ (λ a b, if bool_decide (S a b) then (E2 b) else 1)); auto.
    - real_solver.
    - intros h1 h2 Hh1 Hh2 Hh1h2.
      rewrite -Hε2.
      assert
        (SeriesC (λ a : A, μ1 a * h1 a) <= (SeriesC (λ b : B, μ2 b * h2 b) + SeriesC (λ b : B, μ2 b * E2 b)) + ε1); last by lra.
      rewrite -SeriesC_plus.
      + rewrite (SeriesC_ext _ (λ x : B, μ2 x * (h2 x + E2 x))); [|real_solver].
        transitivity (SeriesC (λ x : B, μ2 x * (Rmin 1 (h2 x + E2 x))) + ε1); last first.
        * apply Rplus_le_compat; [|lra].
          apply SeriesC_le'.
          ** intros a.
             apply Rmult_le_compat_l; auto.
          ...
          ** apply (ex_seriesC_le _ μ2); auto.
             ...
             *** rewrite -{2}(Rmult_1_r (μ2 b)).
                 apply Rmult_le_compat_l; auto.
                 apply Rmin_l.
          ** apply (ex_seriesC_ext (λ x : B, μ2 x * h2 x + μ2 x * E2 x)); [real_solver|].
             apply ex_seriesC_plus.
             *** apply (ex_seriesC_le _ μ2); auto.
                 real_solver.
             *** apply (ex_seriesC_le _ μ2); auto.
  ...
  Proof.
    intros Hε1 Hε2 Hcoup_fg Hcoup_R.
    apply (ARcoupl_mon_grading _ _ _ (ε1 + (SeriesC (λ b, μ2 b * (Rmin ε2 1))))).
    {
      apply Rplus_le_compat_l.
      rewrite SeriesC_scal_r.
      rewrite -{2}(Rmult_1_l ε2).
    ...
    apply (ARcoupl_dbind_adv_rhs _ _ _ _ R S _ _ (λ b, Rmin ε2 1)); auto.
    - intros; split.
      + apply Rmin_glb; lra.
      + apply Rmin_r.
    - intros a b Rab.
