...
Section couplings_theory.
  ...
  Lemma Mcoupl_eq_elim_dp (μ1 μ2 : distr A) ε :
    Mcoupl μ1 μ2 (=) ε →
    forall (P : A -> Prop),
    SeriesC (λ a : A, if bool_decide (P a) then μ1 a else 0)  <=
    exp ε * SeriesC (λ a : A, if bool_decide (P a) then μ2 a else 0).
  Proof.
    intros Hcoupl P.
    rewrite /Mcoupl in Hcoupl.
...
Section Mcoupl.
  ...
  Lemma Mcoupl_pos_R R ε :
    Mcoupl μ1 μ2 R ε → Mcoupl μ1 μ2 (λ a b, R a b ∧ μ1 a > 0 ∧ μ2 b > 0) ε.
  Proof.
...
Proof.
  ...
  replace (SeriesC (λ a : A, if bool_decide (a = x) then exp ε * μ' a * g x else 0))
    with (exp ε * SeriesC (λ a : A, if bool_decide (a = x) then μ' a * g x else 0)).
  2: rewrite -SeriesC_scal_l ; apply SeriesC_ext ; intros ; case_bool_decide ; field.
