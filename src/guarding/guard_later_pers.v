...
Proof.
  iIntros "[x #g]".
  iMod (guards_extract_persistent X (▷ (T ∗ P)) (▷ P) E F with "g x []") as "[x p]"; trivial.
  { iIntros "[_ latp]". iFrame "latp". }
  
  iDestruct (guards_extract_persistent X (▷ (T ∗ P)) (▷ P) E F with "g x []") as "r"; trivial.
  { iIntros "[_ latp]". iFrame "latp". }
  ...
  iDestruct (guards_transitive F X _ (▷ T) with "g [g1]") as "g3".
    { setoid_rewrite bi.later_sep at 2. iFrame "g1". }
  iDestruct (guards_transitive F X (▷ T) T with "g3 g2") as "g4".
  iMod "r" as "[x p2]". iModIntro. iFrame "x". iNext. 
  iApply (guards_include_pers with "p g4").
Qed.
