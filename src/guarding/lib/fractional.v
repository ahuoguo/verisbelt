...
Require Import guarding.internal.misc_tactics.
...
Section Frac.
  ...
  Definition half γ := own_frac γ (1 / 2)%Qp.

  Lemma frac_split_in_half' (N: namespace) (P: iProp Σ) (E: coPset) :
    (↑N ⊆ E) →
    P ⊢ |={E}=> ∃ (γ: gname) ,
        half γ ∗ half γ ∗ (half γ &&{↑N}&&> ▷ P) ∗ □ (half γ -∗ half γ ={↑N}=∗ ▷ P).
  Proof.
    intros. iIntros "P".
    iMod (frac_alloc2 N with "P") as (γ) "[F [#back #guard]]". { set_solver. }
    rewrite <- Qp.half_half at 2. rewrite <- frac_join. iDestruct "F" as "[F1 F2]".
    iModIntro. iExists γ. iFrame. iSplit. { iApply "guard". }
    iModIntro. iIntros "F1 F2". iCombine "F1 F2" as "F". rewrite frac_join. rewrite Qp.half_half.
    iDestruct ("back" with "F") as "X". iFrame.
  Qed.
  ...
  Lemma frac_split_in_half (N: namespace) (P: iProp Σ) (E: coPset) :
    ...
    P ⊢ |={E}=> ∃ (γ: gname) ,
        half γ ∗ half γ ∗ (half γ &&{↑N}&&> P) ∗ □ (half γ -∗ half γ ={↑N}=∗ P).
  Proof.
    intros. iIntros "P". iMod (frac_split_in_half' N P E with "P") as (γ) "[A [B [#C #D]]]"; trivial.
    iModIntro. iExists γ. iFrame. iSplit.
    { iApply guards_remove_later_rhs. iFrame "C". }
    { iModIntro. iIntros "H1 H2". iMod ("D" with "H1 H2") as "P". iMod "P". iModIntro. iFrame. }
  Qed.
        
  Lemma frac_split_guard_in_half (N: namespace) (G P: iProp Σ) (E: coPset) :
    (Timeless G) →
    (↑N ⊆ E) →
    G -∗ (G &&{↑N}&&> P) -∗ |={E}=> ∃ (γ: gname) ,
        half γ ∗ half γ ∗ (half γ &&{↑N}&&> P) ∗ □ (half γ -∗ half γ ={↑N}=∗ G).
  Proof.
    intros. iIntros "G #guard".
    iMod (frac_split_in_half N G E with "G") as (γ) "[H1 [H2 [C D]]]"; trivial.
    iModIntro. iExists γ. iFrame. iApply (guards_transitive with "C guard").
  Qed.
End Frac.
