...
Section LlftLogic.
  ...
  Proof.
    ...
    iIntros "deadk".
    iDestruct (guards_open (True)%I (▷ (P ∨ llft_dead κ ∗ Cancel γc))%I (↑ NLLFT) (↑ NLLFT) with "Tguard []") as "J". { set_solver. } { done. }
  ...
  Proof.
    ...
    leaf_hyp "incl" rhs to (False)%I as "G2".
    {
      ...
      iApply (llftl_not_own_end κ' with "a"). unseal. iFrame "dead".
    }
    unfold llft_ctx.
  ...
  Lemma llftl_incl_glb κ κ' κ'' :
      κ ⊑ κ' -∗ κ ⊑ κ'' -∗ κ ⊑ (κ' ⊓ κ'').
  Proof.
    apply guards_and_point; [exact: point_prop_llft_alive|].
