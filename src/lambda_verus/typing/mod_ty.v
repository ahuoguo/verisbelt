...
Section typing.
  ...
  Lemma mod_ty_own' {𝔄 𝔅} (fmap : 𝔄 →ₛ 𝔅) (finv : 𝔅 →ₛ 𝔄) `{!@Isoₛ 𝔄 𝔅 fmap finv} ty x d g tid vl :
    ty_own (<{ fmap ; finv }> ty) x d g tid vl ⊢ ty_own ty (finv ~~$ₛ x) d g tid vl.
  ...
  Global Instance mod_ty_type_ne {𝔄 𝔅} (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ} : TypeNonExpansive <{f;fⁱ}>%T.
  Proof.
    split=>/= *; by [apply type_lft_morphism_id_like| |do 3 f_equiv|do 3 f_equiv].
  Qed.

  Global Instance mod_ty_copy {𝔄 𝔅} (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ} ty : Copy ty → Copy (<{f;fⁱ}> ty).
  Proof.
  ...
  Global Instance mod_ty_send {𝔄 𝔅} (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ} ty
    (syn_abstract_consistent : ∀ x x', syn_abstract x = syn_abstract x' →
        syn_abstract (fⁱ ~~$ₛ x) = syn_abstract (fⁱ ~~$ₛ x'))
    : Send ty → Send (<{f;fⁱ}> ty).
  Proof.
    intros [Hphys]. split.
    intros tid tid' x x' Habs. simpl. apply Hphys.
    apply syn_abstract_consistent. trivial.
  Qed.

  Global Instance mod_ty_sync {𝔄 𝔅} (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ} ty : Sync ty → Sync (<{f;fⁱ}> ty).
  Proof.
    intros Hsync. unfold Sync in Hsync. intros tid tid' x d g.
      have Hsync' := Hsync tid tid' (fⁱ ~~$ₛ x) d g. apply Hsync'.
  Qed.

  (* [mod_ty_resolve] / [mod_ty_resolve_just] removed along with the
     [resolve] predicate (prophecy-stripped). *)

  Lemma mod_ty_id {𝔄} (ty: type 𝔄) : <{idₛ;idₛ}>%T ty ≡ ty.
  Proof. split; move=>// *; by rewrite mod_ty_own. Qed.

  Lemma mod_ty_compose {𝔄 𝔅 ℭ}
      (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ}
      (g : 𝔅 →ₛ ℭ) (gⁱ : ℭ →ₛ 𝔅) `{!Isoₛ g gⁱ} ty :
    (<{g;gⁱ}> (<{f;fⁱ}> ty) ≡ <{g ∘ₛ f ; fⁱ ∘ₛ gⁱ}> ty)%T.
  Proof. by split. Qed.

  Lemma mod_ty_in {𝔄 𝔅} E L (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ} ty : subtype E L ty (<{f;fⁱ}> ty) f.
  Proof.
  ...
  Lemma mod_ty_out {𝔄 𝔅} E L (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ} ty :
  ...
  Lemma mod_ty_eqtype {𝔄 𝔅 𝔄' 𝔅'} E L (h: 𝔄 →ₛ 𝔄') (h': 𝔄' →ₛ 𝔄)
      (f : 𝔄 →ₛ 𝔅) (fⁱ : 𝔅 →ₛ 𝔄) `{!Isoₛ f fⁱ}
      (g : 𝔄' →ₛ 𝔅') (gⁱ : 𝔅' →ₛ 𝔄') `{!Isoₛ g gⁱ}
      (ty : type 𝔄) (ty' : type 𝔄') :
