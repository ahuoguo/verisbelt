...
Global Instance compose_iso {A B C} `{!@Iso A B f f'} `{!@Iso B C g g'} : @Iso A C (g ∘ f) (f' ∘ g').
Proof. split; unfold SemiIso; fun_ext; intros x;
  simpl; rewrite semi_iso'; rewrite semi_iso'; trivial.
Qed.
...
Lemma forall_iff {P} (X Y: P → Prop) :
  (∀ (p: P), X p ↔ Y p) →
  ((∀ (p: P), X p) ↔ (∀ (p: P), Y p)).
Proof.
  intros H. split; intros Ha p. { rewrite <- H. apply Ha. } rewrite H. apply Ha.
Qed.
